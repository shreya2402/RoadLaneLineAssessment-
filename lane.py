...
calib_file = r'E:\GeekyBeeAI\Internship project\driving-lane-line-detection\camera_cal\calibration_pickle.p'
...
def create_output_frame(offcenter, pts, undist_ori, fps, curvature, curve_direction, binary_sub, threshold=0.6):
    ...
    whole_frame[220+int(h/2):220+2*int(h/2), 20:20+int(w/2), :] = undist_birdview
    whole_frame[220+int(h/2):220+2*int(h/2), 40+int(w/2):40+w, 0] = cv2.resize((binary_sub*255).astype(np.uint8), (0,0), fx=1/2, fy=1/2)
    whole_frame[220+int(h/2):220+2*int(h/2), 40+int(w/2):40+w, 1] = cv2.resize((binary_sub*255).astype(np.uint8), (0,0), fx=1/2, fy=1/2)
    whole_frame[220+int(h/2):220+2*int(h/2), 40+int(w/2):40+w, 2] = cv2.resize((binary_sub*255).astype(np.uint8), (0,0), fx=1/2, fy=1/2)
    ...
    cv2.putText(whole_frame, "Lane Line Detection", (23,25), font, 0.8, (255,255,0), 1, cv2.LINE_AA)
